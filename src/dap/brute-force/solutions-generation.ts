...
export const generateAllSolutions = (graph: Graph): [][][] => {
  const solutionsForColumnsPerDemands: number[][][] = graph.demands
    .map(generateSolutionsForColumn)
    .map((el: List<List<number>>) => {
      return el.map((innerEl: List<number>) => innerEl.toArray()).toArray();
    })
    .toArray();
